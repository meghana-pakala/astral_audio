"""
- define audio feature targets per planet
...
PLANET_PROFILES = {
    ...
    # MOON - emotional nature, instinctive reactions
    ...
    # JUPITER - growth, abundance, optimism
    ...
    # NEPTUNE - dreams, longing, transcendence
...
# feature weights for track scoring - reflects importance for matching mood
...
def rescore(library_df, base_target, slider_values,
    ...
    # start from slider positions (user adjustments)
    ...
    if liked_uris:
        ...
        if not liked_df.empty:
            ...
            for feat in CONTINUOUS_FEATURES:
                adjusted[feat] += 0.5 * (liked_mean[feat] - adjusted[feat])
    ...
    if disliked_uris:
        ...
        if not disliked_df.empty:
            ...
            for feat in CONTINUOUS_FEATURES:
                adjusted[feat] -= 0.5 * (disliked_mean[feat] - adjusted[feat])

    # clamp to valid range
    for feat in CONTINUOUS_FEATURES:
        adjusted[feat] = float(np.clip(adjusted[feat], 0.0, 1.0))

    # exclude disliked tracks from results
    scoring_df = library_df[~library_df['track_uri'].isin(disliked_uris or [])]
    return score_tracks(scoring_df, adjusted, top_n), adjusted